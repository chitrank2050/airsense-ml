...
4. **Linting (`ruff`)**: Runs `uv run ruff check src/` to find any coding errors or style violations. Unlike `make lint`, it runs in check-only mode and will fail the build if errors are detected.
5. **Formatting (`ruff`)**: Runs `uv run ruff format src/ --check` to ensure the code complies with our formatting standards. It will fail if any files require reformatting.
